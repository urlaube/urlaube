<?php
  ...
  /**
    This is the FileAddon class of the urlau.be CMS.

    The file addon loads content from files.
  ...
  */
  ...
  class FileAddon extends BaseSingleton implements Plugin {
    ...
    protected static function fileToUri($filename) {
      ...
      if (0 === strpos($filename, USER_CONTENT_PATH)) {
        ...
        $result = trail(gc(ROOTURI, null).strtr($filename, DS, US), US);
      }
    ...
    protected static function loadContent($filename, $skipcontent = false, $filter = null) {
      ...
      if (false !== $file) {
        ...
        if (null !== $result) {
          ...
          $result->preset(UPDATE, date(gc(TIMEFORMAT, null), filemtime($filename)));
    ...
    public static function run($name = null, $recursive = false, $skipcontent = false, $filter = null) {
      ...
      if ($recursive) {
        ...
        $dirname = trail(realpath(USER_CONTENT_PATH.nolead($name, DS)), DS);

        // check if the dirname is located in the user content path and represents a folder
        if ((0 === strpos($dirname, USER_CONTENT_PATH)) && is_dir($dirname)) {
        ...
        $filename = realpath(USER_CONTENT_PATH.notrail(nolead($name, DS), DS).static::EXTENSION);

        // check if the filename is located in the user content path, really ends with the extension and is a file
        if ((0 === strpos($filename, USER_CONTENT_PATH)) &&
            istrail($filename, static::EXTENSION) && is_file($filename)) {
          // load a single file
  ...
  // register plugin
  Plugins::register(FileAddon::class, "run", ON_CONTENT);
